...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i{n-1} ; i>=0 ; i--) {
            if(v[i]==0) {
                if((i<n-1) && (v[i+1]==0)) {hsh1[i];continue;}
                hsh1[i];curr=i;continue;
            }
            hsh1[i]=v[i]+hsh1[i+1];
            hsh[i] = {hsh1[i],curr-i+1};
            hsh1[i+1]=0;
        }
        // print(hsh1);
        for(int i{} ; i<n ; i++) {
            if(hsh1[i]!=0) {vec.push_back(hsh[i]);}
        }
        // print(vec);
        // for(auto elem:vec) {cout<<elem.first<<" "<<elem.second<<"\n";}+

        vi ans;
        while(!vec.empty()) {
            int f = vec.back().first;
            int s = vec.back().second;
            s--;
            vec.pop_back();
            while(s--) {
                ans.push_back(0);
