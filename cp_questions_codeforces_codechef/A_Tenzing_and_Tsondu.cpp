    ...
    while(t--) {
        ...
        while((!d1.empty()) && (!d2.empty())) {
            if(x%2 == 0) {
                int x = d1.back();
                int y = d2.front();
                if(x > y) {d1.back()-=d2.front();d2.pop_front();}
                else if(y > x) {d2.front()-=d1.back();d1.pop_back();}
                else {
                    d1.pop_back();
                    d2.pop_front();
                }
                ...
                int x = d1.front();
                int y = d2.back();
                if(x > y) {d1.front()-=d2.back();d2.pop_back();}
                else if(y > x) {d2.back()-=d1.front();d1.pop_front();}
                else {
                    d1.pop_front();
                    d2.pop_back();
                }
        ...
        if(d1.empty() && d2.empty()) {cout<<"Draw\n";continue;}
        if(!d1.empty()) {cout<<"Tsondu\n";continue;}
        cout<<"Tenzing\n";
    }
}