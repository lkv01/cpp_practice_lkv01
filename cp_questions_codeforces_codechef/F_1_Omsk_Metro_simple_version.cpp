...
//         vector<pair<int,int>> minmax(node+1);
//         vector<int> sum(node+1);

//         minmax[1] = {1,1};sum[1] = 1;

//         queue<int> q;
//         q.push(1);
//         while(!q.empty()) {
//             int node = q.front();
//             int min_node = minmax[node].first;
//             int max_node = minmax[node].second;
//             q.pop();
//             for(auto child_pair:graph[node]) {
//                 int child = child_pair.first;
//                 int val = child_pair.second;
//                 sum[child] = sum[node]+val;
//                 int min1 = min(val , min_node+val);
//                 int max1 = max(val , max_node+val);
//                 minmax[child] = {min1,max1};
//                 q.push(child);
//             }
//         }
...
//             int z = sum[node]-wt;
//             if(z == 0) {cout<<"YES\n";continue;}
//             if(minmax[node].first<=z &&  z<=minmax[node].second) {
//                 cout<<"YES\n";
//             } else {
//                 cout<<"NO\n";
//             }
