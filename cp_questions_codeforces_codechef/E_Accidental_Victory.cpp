...
int32_t main() {
    ...
    while(t--) {
        ...
        mi m;
        vi v(n);
        for(int i{} ; i<n ; i++) {cin>>v[i];m[v[i]]+=v[i];}
        
        mi m2 = m;
        for(auto it = m2.begin() ; it!=m2.end() ; it++) {
            if(it == m2.begin()) {continue;}
            auto temp_it = it;temp_it--;
            it->second+=temp_it->second;
        }
        ...
        if(m2.size() == 1) {
            cout<<n<<"\n";
            for(int i{} ; i<n ; i++) {cout<<i+1<<" ";}cout<<"\n";
            continue;
        }
        it--;
        
        unordered_set<int> st;st.insert(it->first);

        for( ; it != m2.begin() ; it--) {
            auto temp_it = it;temp_it--;
            if(temp_it->second < it->first) {break;}
            st.insert(temp_it->first);
        }
